/**
 * pxc-upgraded clone fence. Runs FIRST (init.groovy.d is alphabetical) on
 * every boot. The JENKINS_HOME on this master is restored from a snapshot of
 * production pxc, so at JVM start config.xml still carries prod's EC2 and
 ...
 *  - Jenkins URL to this host, so nothing advertises pxc.cd.percona.com.
 */
...
location.setUrl('https://pxc-upgraded.cd.percona.com/')
location.save()
