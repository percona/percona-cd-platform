...
// jenkins-pxc-master role by the standalone TF block in
// terraform/master-pxc.tf (jenkins-arm-fleet module). The plugin uses the
...
// `privateIpUsed = true` because the us-west-1 master's egress does not
// reliably reach the worker's public IP; master + worker share the same
// TF-created VPC (10.156.0.0/22), so private IP routing works.
//
...
def fleet = new EC2FleetCloud(
    ...
    true,                        // privateIpUsed -- master + worker share the master VPC; public-IP SSH egress is not reliable in us-west-1
