...
netMap = [:]
netMap['eu-central-1b'] = 'subnet-0129be02b1e99d0a1'
netMap['eu-central-1c'] = 'subnet-00369011e9854f743'
...
initMap['docker'] = '''
    ...
    sudo yum -y install java-21-amazon-corretto-headless tzdata-java git docker p7zip cronie || sudo yum -y install java-17-amazon-corretto tzdata-java git docker p7zip cronie
...
initMap['debMap'] = '''
    ...
    # Java 21 on the agent remoting JVM; trixie, jammy and noble carry
    ...
    if [[ ${DEB_VER} == "trixie" ]] || [[ ${DEB_VER} == "jammy" ]] || [[ ${DEB_VER} == "noble" ]]; then
    ...
    # bookworm, bullseye, focal and bionic ship no openjdk-21 package, so the
    # agent JVM is a pinned Temurin 21 JRE tarball symlinked into
    # /usr/local/bin (first on the SSH launcher PATH). Only remoting moves to
    # 21; the distro package above stays the system java.
    case "${DEB_VER}" in
        bookworm|bullseye|focal|bionic)
...
devMap = [:]
devMap['docker']            = '/dev/xvda=:20:true:gp3,/dev/xvdd=:100:true:gp3'
devMap['docker2']           = '/dev/xvda=:20:true:gp3,/dev/xvdd=:80:true:gp3'
devMap['docker-32gb']       = devMap['docker']
devMap['micro-amazon']      = devMap['docker']
devMap['min-amazon-2-x64']  = '/dev/xvda=:30:true:gp3,/dev/xvdd=:80:true:gp3'
...
devMap['min-stretch-x64']   = 'xvda=:30:true:gp3,xvdd=:80:true:gp3'
devMap['min-xenial-x64']    = devMap['min-bionic-x64']
devMap['min-centos-6-x32']  = '/dev/sda=:30:true:gp3,/dev/sdd=:80:true:gp3'
devMap['min-buster-x64']    = '/dev/xvda=:30:true:gp3,/dev/xvdd=:80:true:gp3'
devMap['min-bullseye-x64']  = '/dev/xvda=:30:true:gp3,/dev/xvdd=:80:true:gp3'
devMap['min-bookworm-x64']  = '/dev/xvda=:30:true:gp3,/dev/xvdd=:80:true:gp3'
devMap['min-trixie-x64']    = '/dev/xvda=:30:true:gp3,/dev/xvdd=:80:true:gp3'
