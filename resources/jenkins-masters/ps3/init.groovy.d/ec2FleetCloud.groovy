...
// plugin) as a Jenkins Cloud serving the `docker-32gb-aarch64` label on ps3,
// replacing the classic single-SKU `r6g.2xlarge` SlaveTemplate that previously
// served the same label (retired from cloud.groovy in the same commit).
// Uniform multi-SKU fallback path with ps57 and ps80.
//
// IAM: the `Ec2FleetPluginAutoScaling` policy is attached to the
// jenkins-ps3-master role by the standalone TF block in terraform/master-ps3.tf
// (jenkins-arm-fleet module). The plugin uses the master IAM instance profile
// (`awsCredentialsId = ""`).
//
// SSH: the `percona-jenkins` private-key credential is loaded into ps3; it
// matches the AWS key pair on the Launch Template (`key_name = "percona-jenkins"`).
// `privateIpUsed = true` because the eu-west-1 master's egress does not reliably
// reach the worker's public IP; master + worker share the same VPC, so private
// IP routing works.
//
...
def fleet = new EC2FleetCloud(
    ...
    true,                        // privateIpUsed -- master + worker share the master VPC; public-IP SSH egress is not reliable in eu-west-1
