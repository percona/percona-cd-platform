...
```dockerfile
FROM jenkins/jenkins:2.568.3-lts-jdk21@sha256:<DIGEST>
```

2.568.3 is the current LTS with the open core security batches fixed
...
```bash
docker buildx imagetools inspect jenkins/jenkins:2.568.3-lts-jdk21 \
...
`2.568.3-a1b2c3d`), not by a true `@sha256:` digest. The upstream `jenkins`
