...
def enumerate_masters() -> list[dict]:
    ...
    inst_dir = os.path.join(repo_root(), "resources/jenkins/master/instances")
    k8s_names: list[str] = []
    ...
    for tf in sorted(glob.glob(os.path.join(repo_root(), "terraform", "master-*.tf"))):
...
def mimir_query(port: int, query: str) -> dict:
    url = f"http://127.0.0.1:{port}/prometheus/api/v1/query?" + urllib.parse.urlencode(
        {"query": query}
    )
    return http_json(url, headers={"X-Scope-OrgID": TENANT}, timeout=20)
...
def main(argv: list[str] | None = None) -> int:
    ...
    bad = [str(r["label"]) for r in rows if r["status"] != "OK"]
    if bad:
