...
| # | Item | Where | Source |
|---|---|---|---|
| 1 | **Access entries with `authenticationMode=API`**; `bootstrapClusterCreatorAdminPermissions=false`; the baseline cluster-admin entry is committed and resolved dynamically from the IAM Identity Center AdministratorAccess role (no ARN literals), `var.access_entries` adds overrides ([ADR 0036](adr/0036-operator-allowlists-in-ssm-and-dynamic-sso-access-entry.md)) | `terraform/eks.tf` (`aws_iam_roles` lookup, `access_entries`, `authentication_mode`) | [IAM best practices](https://docs.aws.amazon.com/eks/latest/best-practices/identity-and-access-management.html) |
| 2 | **`publicAccessCidrs` allowlist** on the API endpoint; baseline in SSM parameter `/<cluster>/allowlist/eks-api`, amended via `just allowlist-set` ([runbook](runbooks/eks-api-access.md)) | `terraform/allowlists.tf`, `terraform/eks.tf` (`endpoint_public_access_cidrs`) | [Cluster endpoint access](https://docs.aws.amazon.com/eks/latest/userguide/cluster-endpoint.html) |
...
| 4 | **VPC CNI prefix delegation** (`ENABLE_PREFIX_DELEGATION=true`) — ~4× pod density (prefix mode) on the Graviton MNG nodes | `terraform/eks-addons.tf` (`aws_eks_addon "vpc-cni" { configuration_values = ... }`) | [Prefix mode for Linux](https://docs.aws.amazon.com/eks/latest/best-practices/prefix-mode-linux.html) |
| 5 | **Pin every managed-addon version** + Karpenter AMI alias — no `@latest` anywhere. Half done as of 2026-06-11: AMI alias and MNG AMIs are pinned, but `var.addon_versions` is still an empty map, so the six managed addons resolve to channel defaults at apply time. Populate the map from the live `describe-addon` versions. | `terraform/eks-addons.tf` (`addon_version`), `terraform/variables.tf` (`addon_versions`), `resources/addons/karpenter/templates/ec2nodeclass-default.yaml` (`amiSelectorTerms.alias`) | [Cluster upgrades](https://docs.aws.amazon.com/eks/latest/best-practices/cluster-upgrades.html) |
...
| 9 | in progress | **Customer-managed KMS CMK** for cluster secrets envelope encryption + EBS volume encryption. Secrets half done (module-created CMK with rotation, `terraform/eks.tf` `create_kms_key` + `encryption_config`, live-verified 2026-06-11). EBS half open: StorageClasses and the Karpenter EC2NodeClass set `encrypted: true` with no `kmsKeyId`, so volumes use the AWS-managed key. | `terraform/eks.tf` (done), StorageClass `parameters.kmsKeyId` (open) | [Envelope encryption](https://docs.aws.amazon.com/eks/latest/userguide/envelope-encryption.html) |
...
| 13 | done | **Karpenter** `consolidationPolicy: WhenEmptyOrUnderutilized`, `disruption.budgets`, `expireAfter: 720h`, NodePool `limits.cpu`, all set on the default pool (live-verified 2026-06-11). The lgtm-stateful pool deliberately diverges (`WhenEmpty`, `expireAfter: Never`, zero-node budgets). The Jenkins do-not-disrupt sub-item is N/A: no in-cluster build pods exist, the ps3-k8s controller is guarded by dedicated MNG placement, and LGTM pods carry the annotation. | `resources/addons/karpenter/templates/nodepool-default.yaml` | [Karpenter](https://docs.aws.amazon.com/eks/latest/best-practices/karpenter.html) |
| 14 | done | **VolumeSnapshotClass** for EBS CSI (independent of AWS Backup; useful for Velero / ad-hoc). `ebs-csi-retain` (Retain policy, reaper tags) plus the managed snapshot-controller addon and snapscheduler, live-verified 2026-06-11. | `resources/addons/storageclass-gp3/templates/volumesnapshotclass.yaml`, `terraform/eks-addons.tf` | [EBS CSI driver](https://github.com/kubernetes-sigs/aws-ebs-csi-driver/blob/master/docs/install.md) |
| 15 | open | **Alertmanager routing** — Slack `#opensource-jenkins` / PagerDuty. The alertmanager pod runs and a fallback config exists live, but it is a null route (single default receiver, no Slack/PagerDuty/webhook), so every alert dead-ends (verified 2026-06-11). | `resources/addons/mimir/values.yaml` (`alertmanager.fallbackConfig`) | [Mimir Alertmanager](https://grafana.com/docs/mimir/latest/references/architecture/components/alertmanager/) |
...
| 18 | done | **Bearer-token auth on Alloy gateway**: the NGINX sidecar validates `Authorization: Bearer` against the ESO-synced Secrets Manager value on all three receiver ports, strips the header, and proxies loopback (ADR 0013 Layer 1, ALB CIDR allowlist is Layer 2). Live-verified 2026-06-11 on both gateway pods. Residual: the recommended master/worker two-bearer split is not implemented, one bearer serves all senders. | `resources/addons/alloy-gateway/templates/configmap-nginx-auth.yaml`, `templates/external-secret-bearer.yaml` | [ADR 0013](adr/0013-push-from-masters-with-nginx-bearer.md) |
...
| 22 | — | **~~Multi-AZ NAT-GW~~** (reversed). H-2 (PR #14) adopted `one_nat_gateway_per_az = true`, then [ADR 0020](adr/0020-lgtm-single-az-collapse.md) collapsed back to a single NAT in us-east-1a on cost grounds (saves ~$200/mo, all stateful workloads already 1a-pinned, S3 gateway endpoint absorbs the bulk of egress). Live posture is `single_nat_gateway = true` with an accepted us-east-1a egress SPOF. See [Explicitly deferred](#explicitly-deferred-not-gaps--documented-choices). | `terraform/vpc.tf` | [Networking](https://docs.aws.amazon.com/eks/latest/best-practices/networking.html) |
...
| 24 | in progress | **EC2 master DATA-volume snapshots in Terraform**. DLM daily, retain 14, plus a cross-region copy to us-east-1, one policy per master region selecting `snapshot-policy=jenkins-master`. Replaces 12 hand-made console policies (weekly/retain-4, single-region). Applied 2026-06-18: one ENABLED policy per master region is live and coverage is verified across all master data volumes (pg included). Pending one daily cycle: the first daily snapshot firing, the cross-region copy verification in us-east-1, and the retirement of the legacy console policies. Complements #14 (in-cluster snapscheduler) and `backup.tf` (cluster EBS). RPO ratified in [ADR 0037](adr/0037-master-ebs-snapshot-rpo.md). | `terraform/ebs-snapshots.tf`, `terraform/modules/dlm-snapshot/` | [ADR 0037](adr/0037-master-ebs-snapshot-rpo.md) |
...
- **Single NAT-GW (us-east-1a)**, by decision. Multi-AZ NAT was adopted in H-2 (PR #14, [ADR 0011](adr/0011-robustness-pass.md) §H4) then reversed by [ADR 0020](adr/0020-lgtm-single-az-collapse.md) on cost grounds (all stateful workloads already 1a-pinned, the S3 gateway endpoint absorbs the bulk of egress). Accepted SPOF: an us-east-1a outage black-holes pod egress. Item #22 tracks the history.
