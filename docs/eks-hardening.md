...
|---|---|---|---|---|
| 6 | ✅ | **PodDisruptionBudgets** across HA components — ArgoCD (server/repo/AS/controller), AWS LBC, external-dns, ESO, Mimir/Loki/Tempo (distributor/ingester/querier/qf/store-gw/AM/ruler), Grafana, Alloy gateway, Alertmanager, Prometheus, kube-state-metrics. Closed in [PR #13 (H-1)](https://github.com/nogueiraanderson/percona-ci-platform/pull/13) + [PR #14 (H-2)](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). | per-addon `values.yaml`, `terraform/argocd.tf` | [Reliability](https://docs.aws.amazon.com/eks/latest/best-practices/reliability.html) |
| 7 | ✅ | **PriorityClasses** — three-tier custom hierarchy (`platform-system-critical=1B`, `platform-stateful-high=100k`, `platform-default=0`) deployed as its own ApplicationSet addon at sync-wave −100. Closed in [PR #13 (H-1)](https://github.com/nogueiraanderson/percona-ci-platform/pull/13). See [ADR 0011](adr/0011-robustness-pass.md) §H1. | `resources/addons/priorityclasses/` | [Reliability](https://docs.aws.amazon.com/eks/latest/best-practices/reliability.html) |
...
| 10 | ✅ | **Karpenter AMI alias pinned** (`al2023@v20260423`) — no `@latest` anywhere. Closed in [PR #13 (H-1)](https://github.com/nogueiraanderson/percona-ci-platform/pull/13). IMDSv2 + hop-limit=1 still pending separately. | `resources/addons/karpenter/nodepools/ec2nodeclass.yaml` | [Cluster upgrades](https://docs.aws.amazon.com/eks/latest/best-practices/cluster-upgrades.html) |
| 11 | ✅ | **VPC endpoints** — S3 gateway (free) + ecr.api / ecr.dkr / sts / ec2 interface endpoints (the high-leverage scaling-critical APIs). Long-tail (SQS, Secrets Manager, KMS, ELB, SSM, Logs) deliberately deferred. Closed in [PR #14 (H-2)](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). See [ADR 0011](adr/0011-robustness-pass.md) §H7. | `terraform/vpc.tf` (`module.vpc_endpoints`) | [Cost-opt networking](https://docs.aws.amazon.com/eks/latest/best-practices/cost-opt-networking.html) |
...
| 15 | ⬜ | **Alertmanager routing** — Slack `#opensource-jenkins` / PagerDuty (Mimir's built-in Alertmanager ships empty config) | `resources/addons/mimir/values.yaml` (`alertmanager.fallbackConfig`) | [Mimir Alertmanager](https://grafana.com/docs/mimir/latest/references/architecture/components/alertmanager/) |
...
| 20 | ✅ | **S3 Object Lock (Compliance, 7 d)** on the LGTM buckets — blocks delete/overwrite even by account root for retention window. Anti-ransomware / anti-runaway-compactor. Closed in [PR #14 (H-2)](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). See [ADR 0011](adr/0011-robustness-pass.md) §H5. | `terraform/lgtm-storage.tf` (`object_lock_enabled`, `aws_s3_bucket_object_lock_configuration`) | [Object Lock](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock.html) |
| 21 | ⬜ | **SAML cert rotation tracking** for Grafana — PrometheusRule on cert expiry (synced to the Mimir ruler via Alloy `mimir.rules.kubernetes`), calendar reminder 30 d before, runbook in `docs/runbooks/grafana-saml-cutover.md` | new PrometheusRule under `resources/addons/grafana/templates/` plus runbook | upstream Grafana SAML |
| 22 | ✅ | **Multi-AZ NAT-GW** (`one_nat_gateway_per_az = true`) — single NAT-GW had been documented as deferred but reclassified as production-blocker in 2026-05 self-audit; AZ outage in NAT's AZ would otherwise black-hole all egress. Closed in [PR #14 (H-2)](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). See [ADR 0011](adr/0011-robustness-pass.md) §H4. | `terraform/vpc.tf` | [Networking](https://docs.aws.amazon.com/eks/latest/best-practices/networking.html) |
| 23 | ✅ | **Native S3 state locking** (`use_lockfile = true`, OpenTofu 1.10+) — DynamoDB lock table removed entirely from bootstrap, IAM, and runbooks. Closed in [PR #14 (H-2)](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). See [ADR 0011](adr/0011-robustness-pass.md) §H6. | `terraform/backend.tf` | [S3 backend](https://opentofu.org/docs/language/settings/backends/s3/) |
...
- ~~**Multi-AZ NAT-GW**~~ — adopted per [ADR 0011](adr/0011-robustness-pass.md) §H4 / [PR #14](https://github.com/nogueiraanderson/percona-ci-platform/pull/14). Item #22 above tracks the implementation.
