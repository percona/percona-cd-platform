...
`percona` lands as Viewer (read-only). `ALLOWED_GROUPS` rejects users in
neither group at the token-validation step.

To add Editor capability later, register a third Duo group (e.g.
