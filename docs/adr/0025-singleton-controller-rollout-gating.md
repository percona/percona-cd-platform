...
Config changes (JCasC / ConfigMap clouds/templates/labels) are exempt: they reload live via the chart sidecar or `POST /configuration-as-code/reload`, no restart, and may be automatic. The exception is a config change that needs a NEW plugin, which is really an image change and inherits this gating. A second bounded exception is the stateless `jenkins-mcp` gateway addon, whose image auto-deploys via a CI-authored bump PR ([ADR 0038](0038-jenkins-mcp-image-autodeploy.md)): it preserves this ADR's reviewed-signed-git-change principle (the bump is still a human-merged, GitHub-verified PR), only the PR authoring is automated, and it applies to that stateless addon alone, never to a singleton controller.
