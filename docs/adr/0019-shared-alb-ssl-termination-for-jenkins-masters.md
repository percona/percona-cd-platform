...
**Amendment (2026-05-18, ps3 cutover):** path between the proxy pod and the master is cross-region VPC peering, not a public NAT hairpin. See "Decision" Mode B and "Consequences" sections, both updated below; the peering blocks in `terraform/master-<host>.tf` and the per-host CIDR notes capture the operational shape.
**Amendment (2026-06-11, operator access):** the "EC2 Instance Connect Endpoint is preferred" note in the Decision was never built. Operator access is EC2 Instance Connect over the existing SSM tunnel, with no EICE resource. See [ADR 0032](0032-ec2-instance-connect-over-ssm-for-operator-ssh.md), which supersedes that line.
...
Internet
  ...
  -> Cross-region VPC peering (peering blocks in terraform/master-<host>.tf)
...
- Per-master cutover is a 1-PR-per-side + 1-tfvars-edit + 1-runbook-walk pattern (percona-ci-platform peering blocks in `terraform/master-<host>.tf` + `local.auto.tfvars` origin target; `Percona-Lab/jenkins-pipelines` single PR that renumbers CIDR if needed, strips openresty/certbot, tightens SG to `10.220.0.0/16:8080`, drops CF JDNSRecord). Blast radius is contained to one master at a time.
...
- `origin-<host>` Route53 records are driven by a tag-filtered `data.aws_instances.<host>_master` discovery in `master-<host>.tf` (matches the pattern already used for VPC + route-table lookups). `tofu apply` re-resolves the master's private IP automatically; SpotFleet replacements, AZ failovers and renumbers self-heal without manual intervention. `var.jenkins_origin_targets` remains as an explicit operator override (rare: debugging, parked master, manual cutover). Edge case: during a ~30-90s SpotFleet replacement window where no instance is in `running` state, both sources are null and apply errors — acceptable because we don't apply mid-rotation; operator can set the override IP if unattended apply is needed.
- **ALB access logs** are enabled on the shared `jenkins-cd` ALB (the internet-facing SSO + platform-UI front door), delivered to `${cluster_name}-alb-logs` under the `jenkins-cd` prefix with 90-day lifecycle expiry (`terraform/alb-access-logs.tf`, attribute set on the argocd Ingress in `terraform/argocd.tf`). The dedicated `jenkins-masters` ALB is configured to log to the same bucket under the `jenkins-masters` prefix (annotation rendered identically on the `resources/addons/jenkins-ingress` Ingresses, wired 2026-06-13). Remaining Well-Architected gap in this class: the absent VPC Flow Logs.
