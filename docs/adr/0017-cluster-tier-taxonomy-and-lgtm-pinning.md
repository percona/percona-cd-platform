...
**Amended by:** [ADR 0042](0042-lgtm-stateful-arm64.md) (lgtm-stateful pool) and [ADR 0043](0043-platform-wide-arm64-migration.md) (the MNGs) move this tier to arm64 Graviton. The amd64 families below are the pre-migration record.
...
| `jenkins-master` | `jenkins-master` | `workload.percona.com/tier=jenkins-master:NoSchedule` | EKS MNG `jenkins_master` -- `m6a.xlarge` ×1 us-east-1a |
...
> **`jenkins-master` tier — note (2026-05-31).** The tier definition (label/taint) is unchanged; its backing MNG churned: the original `jenkins_system` MNG was **removed 2026-05-13** ([ADR 0008](0008-managed-ng-for-stateful-system-workloads.md)), then `jenkins_master` (`m6a.xlarge` ×1, us-east-1a, this taint+label) was **added 2026-05-31** for the singleton-controller pilot ([ADR 0025](0025-singleton-controller-rollout-gating.md)) and **committed** to `terraform/eks.tf` (PR #66, `035c1ee`). `tofu plan` shows it in state with no changes, and the AWS CLI confirms the live NG matches the committed spec. No drift.
...
- `kube-prometheus-stack` is uninstalled (ADR 0016) but its PVCs are retained on `gp3-monitoring-1a-retain`. Reinstall plan needs explicit decision on whether to move to multi-AZ. **Update 2026-05-28:** the retained PVCs and their EBS volumes (4× 10 GiB) were deleted; no reinstall planned (LGTM is the single observability stack).
