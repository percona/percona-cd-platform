# 0020 — LGTM single-AZ, single-replica collapse
...
4. Added `persistentVolumeClaimRetentionPolicy` to the chart values so future flips do not leak: PR [#11](https://github.com/percona/percona-cd-platform/pull/11) (commit `86133ae`, 2026-05-27), carried over from the pre-move repo's PR 76. The 17 orphan PVCs from the original migration were swept via the [runbook](../runbooks/lgtm-orphan-pvc-sweep.md); the Released Grafana PV was swept manually since `gp3-monitoring-1a-retain` blocks the cleanup Lambda.

## Verification

- `helm template --kube-version 1.35.0` renders `persistentVolumeClaimRetentionPolicy: {whenDeleted: Delete, whenScaled: Delete}` on all seven target StatefulSets (mimir-distributed 6.0.6, loki 7.0.0, tempo-distributed 1.61.3).
- Live, after the ArgoCD sync of PR 11: `kubectl get sts -n {mimir,loki,tempo}` shows `whenDeleted: Delete` on Mimir ingester/store-gateway/compactor/alertmanager, Loki ingester/compactor, and Tempo ingester.
