...
carved from the /22 (AZ indexes 1 and 2 by default; psmdb adds index 0),
...
- Hub-side routes (`<master CIDR> -> pcx`) are added to the **private**
  route tables only; the proxy and reconciler pods run on private-subnet
...
- The reconciler CronJob runs every minute (staleness bound), discovers the
  ...
  matches writes an empty slice (proxy serves the 503 page); multiple
...
- Each EC2 master attaches three SGs: the VPC default (intra-VPC worker
  ...
  `ssh_allowed_cidrs`, a six-CIDR operator baseline; pmm adds one more).
...
- Shell access is SSM Session Manager (agent dials out :443; no inbound
