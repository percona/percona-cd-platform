# Owner: mysql
# ps80-upgraded.cd.percona.com -- FENCED blue/green clone of ps80 for the
# Jenkins core 2.568.2 + Java 21 upgrade validation (core security advisories
# 2026-06-10 and 2026-08-05). Boots production ps80's real JENKINS_HOME from a
# snapshot-restored data volume, on the upgraded core, with clouds, agents,
# triggers and executors fenced off by
# resources/jenkins-masters/ps80-upgraded/init.groovy.d/ (a-fence.groovy and
...
# if the groovy fence loses a race. Production ps80 stays untouched.
#
# TEMPORARY: delete this file (and the addon host entries) after the ps80
# core upgrade lands.
#
...
#        --snapshot-id <newest completed DLM snapshot of vol-09c0b26ce75129c24> \
#        --volume-type gp3 \
#        --tag-specifications 'ResourceType=volume,Tags=[{Key=Name,Value=jenkins-ps80-upgraded},{Key=iit-billing-tag,Value=jenkins-ps80-upgraded},{Key=PerconaKeep,Value=True}]'
#   2. tofu -chdir=terraform import module.ps80_upgraded.aws_ebs_volume.data vol-XXXX (raw-tofu exception)
...
module "ps80_upgraded" {
  source    = "./modules/jenkins-master"
  providers = { aws = aws.us-west-2 }

  hostname             = "ps80-upgraded.cd.percona.com"
  ...
  launch_template_name = "PS80UpgradedMasterTemplate"
  ...
  # JENKINS_HOME stays at the snapshot's directory name (/mnt/ps80.cd.percona.com)
  ...
  # production ps80's series.
  jenkins_home_dirname = "ps80.cd.percona.com"

  # Same volume geometry as the snapshot source (300 GiB gp3 in us-west-2b).
  ebs_size = 300
  az_index = 1
  ...
  extra_master_inline_policies = [
    {
      ...
      json = data.aws_iam_policy_document.ps80_upgraded_alloy_bearer_read.json
    },
    ...
    # lists production ps80's clouds cannot run or terminate EC2 instances.
    {
      name = "DenyWorkerProvisioning"
      json = data.aws_iam_policy_document.ps80_upgraded_deny_provisioning.json
    },
  ]
  ...
  init_groovy_files = {
    for f in fileset("${path.module}/../resources/jenkins-masters/ps80-upgraded/init.groovy.d", "*.groovy") :
    f => file("${path.module}/../resources/jenkins-masters/ps80-upgraded/init.groovy.d/${f}")
  }
  init_groovy_sync_schedule = "rate(30 minutes)"
}

# No arm-fleet sibling: the clone provisions no workers, and the shared ASG
# (jenkins-ps80-arm-graviton) belongs to production ps80.

data "aws_iam_policy_document" "ps80_upgraded_alloy_bearer_read" {
  statement {
...
data "aws_iam_policy_document" "ps80_upgraded_deny_provisioning" {
  statement {
...
# Cross-region VPC peering EKS (us-east-1) <-> ps80-upgraded (us-west-2),
# so the in-cluster jenkins-ingress nginx can reach the master's private IP on
# :8080. Mirrors the ps80 peering. CIDRs: EKS 10.220.0.0/16, clone 10.159.0.0/22.

resource "aws_vpc_peering_connection" "ps80_upgraded" {
  vpc_id      = module.vpc.vpc_id
  peer_vpc_id = module.ps80_upgraded.vpc_id
  peer_region = "us-west-2"
  auto_accept = false

  tags = {
    Name = "${local.cluster_name}-to-jenkins-ps80-upgraded"
  }
}

resource "aws_vpc_peering_connection_accepter" "ps80_upgraded" {
  provider                  = aws.us-west-2
  vpc_peering_connection_id = aws_vpc_peering_connection.ps80_upgraded.id
  auto_accept               = true
...
resource "aws_route" "eks_private_to_ps80_upgraded" {
  for_each                  = toset(module.vpc.private_route_table_ids)
  route_table_id            = each.value
  destination_cidr_block    = module.ps80_upgraded.vpc_cidr
  vpc_peering_connection_id = aws_vpc_peering_connection.ps80_upgraded.id
}

resource "aws_route" "ps80_upgraded_to_eks" {
  provider = aws.us-west-2
  ...
  for_each                  = { for idx, rt in module.ps80_upgraded.private_route_table_ids : tostring(idx) => rt }
  route_table_id            = each.value
