# Owner: mysql
# pxc-upgraded.cd.percona.com -- FENCED blue/green clone of pxc for the
# Jenkins core 2.568.2 + Java 21 upgrade validation (core security advisories
# 2026-06-10 and 2026-08-05). Boots production pxc's real JENKINS_HOME from a
# snapshot-restored data volume, on the upgraded core, with clouds, agents,
# triggers and executors fenced off by
# resources/jenkins-masters/pxc-upgraded/init.groovy.d/ (a-fence.groovy and
...
# if the groovy fence loses a race. Production pxc stays untouched.
#
# TEMPORARY: delete this file (and the addon host entries) after the pxc
# core upgrade lands.
#
# Data volume is adopted, not created (the ps57 retained-volume precedent, master-ps57.tf):
#   1. aws ec2 create-volume --region us-west-1 --availability-zone us-west-1b \
#        --snapshot-id <newest completed DLM snapshot of vol-03b3852ad6dd6c553> \
#        --volume-type gp2 \
#        --tag-specifications 'ResourceType=volume,Tags=[{Key=Name,Value=jenkins-pxc-upgraded},{Key=iit-billing-tag,Value=jenkins-pxc-upgraded},{Key=PerconaKeep,Value=True}]'
#   2. tofu -chdir=terraform import module.pxc_upgraded.aws_ebs_volume.data vol-XXXX (raw-tofu exception)
...
module "pxc_upgraded" {
  ...
  hostname             = "pxc-upgraded.cd.percona.com"
  short_name           = "jenkins-pxc-upgraded"
  team                 = "mysql"
  ...
  launch_template_name = "PXCUpgradedMasterTemplate"
  ...
  # JENKINS_HOME stays at the snapshot's directory name (/mnt/pxc.cd.percona.com)
  ...
  # production pxc's series.
  jenkins_home_dirname = "pxc.cd.percona.com"

  # Same volume geometry as the snapshot source (300 GiB gp2 in us-west-1b).
  ebs_size = 300
  ebs_type = "gp2"
  az_index = 1
  ...
  extra_master_inline_policies = [
    {
      ...
      json = data.aws_iam_policy_document.pxc_upgraded_alloy_bearer_read.json
    },
    ...
    # lists production pxc's clouds cannot run or terminate EC2 instances.
    {
      name = "DenyWorkerProvisioning"
      json = data.aws_iam_policy_document.pxc_upgraded_deny_provisioning.json
    },
  ]
  ...
  # Fence set, NOT pxc's files: a-fence.groovy clears clouds/agents/executors
  ...
  init_groovy_files = {
    for f in fileset("${path.module}/../resources/jenkins-masters/pxc-upgraded/init.groovy.d", "*.groovy") :
    f => file("${path.module}/../resources/jenkins-masters/pxc-upgraded/init.groovy.d/${f}")
  }
  init_groovy_sync_schedule = "rate(30 minutes)"
}

# No arm-fleet sibling: the clone provisions no workers, and the shared ASG
# (jenkins-pxc-arm-graviton) belongs to production pxc.

data "aws_iam_policy_document" "pxc_upgraded_alloy_bearer_read" {
  statement {
...
data "aws_iam_policy_document" "pxc_upgraded_deny_provisioning" {
  statement {
...
# Cross-region VPC peering EKS (us-east-1) <-> pxc-upgraded (us-west-1),
# so the in-cluster jenkins-ingress nginx can reach the master's private IP on
# :8080. Mirrors the pxc peering. CIDRs: EKS 10.220.0.0/16, clone 10.160.0.0/22.

resource "aws_vpc_peering_connection" "pxc_upgraded" {
  vpc_id      = module.vpc.vpc_id
  peer_vpc_id = module.pxc_upgraded.vpc_id
  peer_region = "us-west-1"
  auto_accept = false

  tags = {
    Name = "${local.cluster_name}-to-jenkins-pxc-upgraded"
  }
}

resource "aws_vpc_peering_connection_accepter" "pxc_upgraded" {
  provider                  = aws.us-west-1
  vpc_peering_connection_id = aws_vpc_peering_connection.pxc_upgraded.id
  auto_accept               = true
...
resource "aws_route" "eks_private_to_pxc_upgraded" {
  for_each                  = toset(module.vpc.private_route_table_ids)
  route_table_id            = each.value
  destination_cidr_block    = module.pxc_upgraded.vpc_cidr
  vpc_peering_connection_id = aws_vpc_peering_connection.pxc_upgraded.id
}

resource "aws_route" "pxc_upgraded_to_eks" {
  provider = aws.us-west-1
  ...
  for_each                  = { for idx, rt in module.pxc_upgraded.private_route_table_ids : tostring(idx) => rt }
  route_table_id            = each.value
