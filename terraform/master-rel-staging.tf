...
module "rel_staging" {
  ...
  extra_master_inline_policies = [
    ...
    # Read its own staging-admin password (terraform/staging-admin.tf).
    {
      ...
      json = data.aws_iam_policy_document.rel_staging_admin_read.json
    },
  ]
...
data "aws_iam_policy_document" "rel_staging_admin_read" {
  statement {
    sid       = "StagingAdminPasswordRead"
    effect    = "Allow"
    actions   = ["ssm:GetParameter"]
    resources = [aws_ssm_parameter.staging_admin_password["rel"].arn]
  }
}
