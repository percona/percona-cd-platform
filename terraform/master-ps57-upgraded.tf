# Owner: mysql
# ps57-upgraded.cd.percona.com -- FENCED blue/green clone of ps57 for the
# Jenkins core 2.568.2 + Java 21 upgrade validation (core security advisories
# 2026-06-10 and 2026-08-05). Boots production ps57's real JENKINS_HOME from a
# snapshot-restored data volume, on the upgraded core, with clouds, agents,
# triggers and executors fenced off by
# resources/jenkins-masters/ps57-upgraded/init.groovy.d/ (a-fence.groovy and
...
# if the groovy fence loses a race. Production ps57 stays untouched.
#
# TEMPORARY: delete this file (and the addon host entries) after the ps57
# core upgrade lands.
#
# Data volume is adopted, not created (the ps57 retained-volume precedent):
...
module "ps57_upgraded" {
  source    = "./modules/jenkins-master"
  providers = { aws = aws.eu-central-1 }

  hostname             = "ps57-upgraded.cd.percona.com"
  short_name           = "jenkins-ps57-upgraded"
  team                 = "mysql"
  ...
  launch_template_name = "PS57UpgradedMasterTemplate"
  ...
  # JENKINS_HOME stays at the snapshot's directory name (/mnt/ps57.cd.percona.com)
  ...
  # production ps57's series.
  jenkins_home_dirname = "ps57.cd.percona.com"

  # Same volume geometry as the snapshot source (100 GiB gp2 in eu-central-1b).
  ebs_size = 100
  ebs_type = "gp2"
  az_index = 1
  ...
  extra_master_inline_policies = [
    {
      ...
      json = data.aws_iam_policy_document.ps57_upgraded_alloy_bearer_read.json
    },
    ...
    # lists production ps57's clouds cannot run or terminate EC2 instances.
    {
      name = "DenyWorkerProvisioning"
      json = data.aws_iam_policy_document.ps57_upgraded_deny_provisioning.json
    },
  ]
  ...
  # Fence set, NOT ps57's files: a-fence.groovy clears clouds/agents/executors
  ...
  init_groovy_files = {
    for f in fileset("${path.module}/../resources/jenkins-masters/ps57-upgraded/init.groovy.d", "*.groovy") :
    f => file("${path.module}/../resources/jenkins-masters/ps57-upgraded/init.groovy.d/${f}")
  }
  init_groovy_sync_schedule = "rate(30 minutes)"
}

# No arm-fleet sibling: the clone provisions no workers, and the shared ASG
# (jenkins-ps57-arm-graviton) belongs to production ps57.

data "aws_iam_policy_document" "ps57_upgraded_alloy_bearer_read" {
  statement {
...
data "aws_iam_policy_document" "ps57_upgraded_deny_provisioning" {
  statement {
...
# Cross-region VPC peering EKS (us-east-1) <-> ps57-upgraded (eu-central-1),
# so the in-cluster jenkins-ingress nginx can reach the master's private IP on
# :8080. Mirrors the ps57 peering. CIDRs: EKS 10.220.0.0/16, clone 10.158.0.0/22.

resource "aws_vpc_peering_connection" "ps57_upgraded" {
  vpc_id      = module.vpc.vpc_id
  peer_vpc_id = module.ps57_upgraded.vpc_id
  peer_region = "eu-central-1"
  auto_accept = false

  tags = {
    Name = "${local.cluster_name}-to-jenkins-ps57-upgraded"
  }
}

resource "aws_vpc_peering_connection_accepter" "ps57_upgraded" {
...
resource "aws_route" "eks_private_to_ps57_upgraded" {
  for_each                  = toset(module.vpc.private_route_table_ids)
  route_table_id            = each.value
  destination_cidr_block    = module.ps57_upgraded.vpc_cidr
  vpc_peering_connection_id = aws_vpc_peering_connection.ps57_upgraded.id
}
