...
# Cutover-only lookups + one-shot import blocks: adopt the CFN resources
# retained at delete-stack, resolved by tags and relationships instead of
# hardcoded IDs. Remove this whole section once the import apply lands.
data "aws_vpc" "pg_cutover" {
  provider = aws.eu-central-1
  tags     = { Name = "jenkins-pg" }
}

data "aws_subnet" "pg_cutover" {
  provider = aws.eu-central-1
  for_each = { b = "B", c = "C", b2 = "B2", c2 = "C2" }
  vpc_id   = data.aws_vpc.pg_cutover.id
  tags     = { Name = "jenkins-pg-${each.value}" }
}

data "aws_route_table" "pg_cutover" {
  provider  = aws.eu-central-1
  subnet_id = data.aws_subnet.pg_cutover["b"].id
}

data "aws_internet_gateway" "pg_cutover" {
  provider = aws.eu-central-1
  filter {
    name   = "attachment.vpc-id"
    values = [data.aws_vpc.pg_cutover.id]
  }
}

data "aws_vpc_endpoint" "pg_cutover_s3" {
  provider     = aws.eu-central-1
  vpc_id       = data.aws_vpc.pg_cutover.id
  service_name = "com.amazonaws.eu-central-1.s3"
}

data "aws_ebs_volume" "pg_cutover" {
  provider = aws.eu-central-1
  filter {
    name   = "tag:Name"
...
import {
  to = module.pg.aws_vpc.this
  id = data.aws_vpc.pg_cutover.id
}

import {
  to = module.pg.aws_vpc_ipv4_cidr_block_association.secondary[0]
  id = one([for a in data.aws_vpc.pg_cutover.cidr_block_associations : a.association_id if a.cidr_block == "10.145.0.0/21"])
}

import {
  to = module.pg.aws_subnet.this["b"]
  id = data.aws_subnet.pg_cutover["b"].id
}

import {
  to = module.pg.aws_subnet.this["c"]
  id = data.aws_subnet.pg_cutover["c"].id
}

import {
  to = module.pg.aws_subnet.extra["b2"]
  id = data.aws_subnet.pg_cutover["b2"].id
}

import {
  to = module.pg.aws_subnet.extra["c2"]
  id = data.aws_subnet.pg_cutover["c2"].id
}

import {
  to = module.pg.aws_internet_gateway.this
  id = data.aws_internet_gateway.pg_cutover.id
}

import {
  to = module.pg.aws_route_table.this
  id = data.aws_route_table.pg_cutover.id
}

import {
  to = module.pg.aws_route.internet
  id = "${data.aws_route_table.pg_cutover.id}_0.0.0.0/0"
}

import {
  to = module.pg.aws_route_table_association.this["b"]
  id = "${data.aws_subnet.pg_cutover["b"].id}/${data.aws_route_table.pg_cutover.id}"
}

import {
  to = module.pg.aws_route_table_association.this["c"]
  id = "${data.aws_subnet.pg_cutover["c"].id}/${data.aws_route_table.pg_cutover.id}"
}

import {
  to = module.pg.aws_route_table_association.extra["b2"]
  id = "${data.aws_subnet.pg_cutover["b2"].id}/${data.aws_route_table.pg_cutover.id}"
}

import {
  to = module.pg.aws_route_table_association.extra["c2"]
  id = "${data.aws_subnet.pg_cutover["c2"].id}/${data.aws_route_table.pg_cutover.id}"
}

import {
  to = module.pg.aws_vpc_endpoint.s3
  id = data.aws_vpc_endpoint.pg_cutover_s3.id
}

import {
  to = module.pg.aws_ebs_volume.data
  id = data.aws_ebs_volume.pg_cutover.id
}
