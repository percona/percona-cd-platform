# Owner: platform
# IAM role assumed by the build-snapscheduler-image GitHub Actions workflow in
# Percona/percona-cd-platform via OIDC, used ONLY by the push-to-main job to push
# the multi-arch snapscheduler operator image (built from images/snapscheduler/)
# to the percona-cd/snapscheduler ECR repo (terraform/ecr.tf). PR builds DO NOT
...
#   - Push image layers + PutImage to ONLY percona-cd/snapscheduler (immutable
...
data "aws_iam_policy_document" "gha_snapscheduler_image_push_perms" {
  statement {
    sid       = "EcrAuthToken"
    effect    = "Allow"
  ...
  statement {
    sid    = "EcrPushToSnapschedulerRepoOnly"
    effect = "Allow"
    actions = [
    ...
    resources = [aws_ecr_repository.snapscheduler.arn]
  }
}
...
module "gha_snapscheduler_image_push" {
  source = "./modules/github-oidc-role"

  name             = "gha-snapscheduler-image-push"
  role_name_prefix = "${local.cluster_name}-"
  description      = "Assumed by the build-snapscheduler-image GHA workflow (push-to-main job only) in percona/percona-cd-platform via OIDC to push the snapscheduler image to percona-cd/snapscheduler ECR. No PR/tag claims; least-privilege ECR push."

  subject_claims = [
    "repo:percona/percona-cd-platform:ref:refs/heads/main",
  ]

  permissions_policy_json = data.aws_iam_policy_document.gha_snapscheduler_image_push_perms.json
...
output "gha_snapscheduler_image_push_role_arn" {
  description = "Role ARN to set as `role-to-assume` in the build-snapscheduler-image GHA workflow push-to-main job."
  value       = module.gha_snapscheduler_image_push.role_arn
}
