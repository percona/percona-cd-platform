# Owner: platform
# IAM role assumed by the build-jenkins-mcp-image GitHub Actions workflow in
# Percona/percona-cd-platform via OIDC, used ONLY by the push-to-main job to
# push the jenkins-mcp gateway image (built from the vendored source in
# images/jenkins-mcp/) to the percona-cd/jenkins-mcp ECR repo (terraform/ecr.tf).
# PR builds DO NOT assume this role (the PR job carries no id-token), so PR code
# can never publish an image.
#
...
# which would let forked-PR code assume a publish role. A separate, explicitly
# reviewed role is required if a tag/release ever becomes the publish trigger.
#
# Blast radius if an OIDC token leaks before it expires:
#   - Push image layers + PutImage to ONLY percona-cd/jenkins-mcp (immutable
...
data "aws_iam_policy_document" "gha_jenkins_mcp_image_push_perms" {
  statement {
    sid       = "EcrAuthToken"
    effect    = "Allow"
  ...
  statement {
    sid    = "EcrPushToJenkinsMcpRepoOnly"
    effect = "Allow"
    actions = [
      ...
      # provenance + SBOM attestations, so the push 403s without it. Repo-scoped.
      "ecr:BatchGetImage",
    ]
    resources = [aws_ecr_repository.jenkins_mcp.arn]
  }
}
...
module "gha_jenkins_mcp_image_push" {
  source = "./modules/github-oidc-role"

  name             = "gha-jenkins-mcp-image-push"
  role_name_prefix = "${local.cluster_name}-"
  description      = "Assumed by the build-jenkins-mcp-image GHA workflow (push-to-main job only) in percona/percona-cd-platform via OIDC to push the jenkins-mcp gateway image to percona-cd/jenkins-mcp ECR. No PR/tag claims; least-privilege ECR push."

  subject_claims = [
    "repo:percona/percona-cd-platform:ref:refs/heads/main",
  ]

  permissions_policy_json = data.aws_iam_policy_document.gha_jenkins_mcp_image_push_perms.json
...
output "gha_jenkins_mcp_image_push_role_arn" {
  description = "Role ARN to set as `role-to-assume` in the build-jenkins-mcp-image GHA workflow push-to-main job."
  value       = module.gha_jenkins_mcp_image_push.role_arn
}
