...
# stages of the LGTM-only migration (plan:
# ~/.claude/plans/hashed-shimmying-crane.md):
#
#   pre       Snapshot baseline before any migration phase runs.
#   ruler     PrometheusRule sync to Mimir's ruler.
#   post      Read path on Mimir, write path on Alloy, ALB bearer gate.
#   worker    Trigger a Hetzner build on the canary master and prove
#             the provision telemetry lands in both Mimir and Loki.
#   dashboard Grafana dashboards render against Mimir.
#   failsafe  kps remnants gone, no fresh kps series, CRDs persist.
...
mimir_push_probe_public() {
  # mimir-push ALB only accepts WRITES; verify the bearer-auth gate.
...
resolve_grafana_auth() {
  # Try GRAFANA_TOKEN first, then fall back to chart admin/password.
  # Probe both against /api/datasources before reporting success --
  # Grafana hashes the initial admin password into its DB and ignores
  # later env-var rotations unless force_reset_admin_password is set,
  # so the chart Secret can be stale.
  local probe
...
stage_ruler() {
  ...
  if [ "${groups:-0}" -ge 0 ]; then
    stage_pass "ruler reachable" "groups=$groups"
  else
    stage_fail "ruler reachable" "ruler API returned no groups field"
  fi
  if [ "${groups:-0}" -ge 1 ]; then
    stage_pass "ruler has at least one group" "groups=$groups"
  else
    stage_skip "ruler has at least one group" "no PrometheusRule instances exist yet (expected on a brand-new cluster)"
  fi
}
...
stage_dashboard_default() {
  ...
  if [ "$def" = "Mimir" ]; then
    ...
    stage_fail "default datasource is Mimir" "default=$def (expected Mimir)"
  fi
}

stage_failsafe() {
  section "X: kps remnants gone"
  local kps_n
  if kubectl get ns kube-prometheus-stack >/dev/null 2>&1; then
    ...
    if [ "$kps_n" -eq 0 ]; then
      ...
      stage_fail "kube-prometheus-stack ns empty" "pods=$kps_n (expected 0)"
    fi
  else
  ...
  if [ "${fresh:-0}" = "0" ]; then
    ...
    stage_fail "no fresh kps series" "fresh_kps_up=$fresh"
  fi
  local crd_n
  ...
  if [ "${crd_n:-0}" -ge 10 ]; then
    ...
    stage_fail "monitoring.coreos.com CRDs present" "crds=$crd_n (expected >=10)"
  fi
}
