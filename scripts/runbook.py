...
def tf_plan() -> dict:
    ...
    out = subprocess.check_output(
        ["tofu", "-chdir=terraform", "show", "-json", "tfplan"], cwd=ROOT)
...
def evaluate_live(inst: str, src: str, assume_yes: bool):
    if shutil.which("jenkins") is None:
        print(f"jenkins CLI not found. Either wait for the 30 min SSM sync plus the")
        print(f"next JVM start, or evaluate manually:")
...
def rb_template_change(args):
    ...
    src_disk = os.path.join(ROOT, src)
    if not os.path.exists(src_disk):
        tftpl = src_disk + ".tftpl"
        if os.path.exists(tftpl):
            die(f"{src} is templated ({fname}.tftpl); the rendered copy lives in S3. "
    ...
    bad = [(a, acts) for a, acts in changes
    ...
    if not changes:
        ...
        die(f"plan contains changes beyond {inst}'s init-config objects; "
    ...
    print(f"  2. just tf-plan returns 'No changes'.")
...
GUIDED = {
    "master-resize": [
        ("Announce the window in #opensource-jenkins (instance replacement: builds in flight die)", None),
        ("Edit on_demand_instance_type in terraform/master-<inst>.tf, merge the PR", None),
        ("Plan and review: expect the instance to REPLACE and the identity volume to re-attach", ["just", "tf-plan"]),
        ("Apply", ["just", "tf-apply"]),
        ("Wait for the endpoint reconciler to converge (about a minute after :8080 serves), then check the UI", None),
        ("Confirm telemetry", ["just", "check-master-alloy"]),
    ],
    "core-bump": [
        ("Announce the window in #opensource-jenkins (user-data change: instance replacement)", None),
